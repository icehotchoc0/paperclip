...
export function PluginSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("common:settings", "Settings"), href: "/instance/settings/heartbeats" },
      { label: t("pluginManager"), href: "/instance/settings/plugins" },
      { label: plugin?.manifestJson?.displayName ?? plugin?.packageName ?? t("pluginDetails") },
    ]);
  ...
  const statusVariant =
    plugin.status === "ready"
      ...
      : plugin.status === "error"
        ? "destructive"
        : "secondary";
  const pluginDescription = plugin.manifestJson.description || t("noDescriptionProvided");
          ...
          <div className="grid gap-6 xl:grid-cols-[minmax(0,1.2fr)_320px]">
                    ...
                    <>
                      <div>
                        ...
                        {dashboardData.worker ? (
                          <div className="grid grid-cols-2 gap-3 text-sm">
                            ...
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t("pendingRpcs")}</span>
                            ...
                            {dashboardData.worker.totalCrashes > 0 && (
                              <>
                                ...
                                {dashboardData.worker.lastCrashAt && (
                                  <div className="flex justify-between col-span-2">
                                    <span className="text-muted-foreground">{t("lastCrash")}</span>
                          ...
                          <p className="text-sm text-muted-foreground italic">{t("noWorkerProcess")}</p>
                        )}
                      </div>
                      ...
                      <div>
                        ...
                        {dashboardData.recentJobRuns.length > 0 ? (
                          ...
                          <p className="text-sm text-muted-foreground italic">{t("noJobRuns")}</p>
                        )}
                      </div>
                      ...
                      <div>
                        ...
                        {dashboardData.recentWebhookDeliveries.length > 0 ? (
                          ...
                          <p className="text-sm text-muted-foreground italic">{t("noWebhookDeliveries")}</p>
                        )}
                      </div>

                      <div className="flex items-center gap-1.5 border-t border-border/50 pt-2 text-xs text-muted-foreground">
                        <Clock className="h-3 w-3" />
                        {t("lastChecked", { time: new Date(dashboardData.checkedAt).toLocaleTimeString() })}
                      </div>
                    </>
                  ) : (
                    <p className="text-sm text-muted-foreground">
                      {t("runtimeDiagnosticsUnavailable")}
                    </p>
                  )}
                </CardContent>
              </Card>
              ...
              {recentLogs && recentLogs.length > 0 ? (
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>{t("lastNLogEntries", { count: recentLogs.length })}</CardDescription>
                  </CardHeader>
                  <CardContent>
            ...
            <div className="space-y-6">
              <Card>
                ...
                <CardContent>
                  {healthLoading ? (
                    <p className="text-sm text-muted-foreground">{t("checkingHealth")}</p>
                  ) : healthData ? (
                    <div className="space-y-4 text-sm">
                    ...
                    <div className="space-y-3 text-sm text-muted-foreground">
                      ...
                      <p>{t("healthChecksRunOnceReady")}</p>
                      {plugin.lastError ? (
                        <div className="break-words rounded border border-destructive/20 bg-destructive/10 p-2 text-xs text-destructive">
                          {plugin.lastError}
                        </div>
              ...
              <Card>
                ...
                <CardContent>
                  {pluginCapabilities.length > 0 ? (
                    ...
                    <p className="text-sm text-muted-foreground italic">{t("noSpecialPermissions")}</p>
                  )}
                </CardContent>
              </Card>
            </div>
...
function PluginConfigForm({ pluginId, schema, initialValues, isLoading, pluginStatus, supportsConfigTest }: PluginConfigFormProps) {
  ...
  const saveMutation = useMutation({
    ...
    onError: (err: Error) => {
      setSaveMessage({ type: "error", text: err.message || t("failedToSaveConfiguration") });
    },
  });
  ...
  const testMutation = useMutation({
    ...
    onSuccess: (result) => {
      if (result.valid) {
        setTestResult({ type: "success", text: t("configTestPassed") });
      } else {
        setTestResult({ type: "error", text: result.message || t("configTestFailed") });
      }
    },
    onError: (err: Error) => {
      setTestResult({ type: "error", text: err.message || t("configTestFailed") });
    },
  });
