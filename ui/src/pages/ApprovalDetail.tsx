...
export function ApprovalDetail() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("breadcrumb"), href: "/approvals" },
      { label: approval?.id?.slice(0, 8) ?? approvalId ?? t("breadcrumb") },
    ]);
  ...
  const approveMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("approveFailed")),
  });
  ...
  const rejectMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("rejectFailed")),
  });
  ...
  const revisionMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("revisionRequestFailed")),
  });
  ...
  const resubmitMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("resubmitFailed")),
  });
  ...
  const addCommentMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("commentFailed")),
  });
  ...
  if (!approval) return <p className="text-sm text-muted-foreground">{t("approvalNotFound")}</p>;
  ...
  return (
    <div className="space-y-6 max-w-3xl">
        ...
        <div className="border border-green-300 dark:border-green-700/40 bg-green-50 dark:bg-green-900/20 rounded-lg px-4 py-3 animate-in fade-in zoom-in-95 duration-300">
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-start gap-2">
              ...
              <div>
                <p className="text-sm text-green-800 dark:text-green-100 font-medium">{t("approvalConfirmed")}</p>
                <p className="text-xs text-green-700 dark:text-green-200/90">
                  {t("approvalConfirmedDescription")}
                </p>
              </div>
            </div>
            <Button
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        ...
        <div className="text-sm space-y-1">
          {approval.requestedByAgentId && (
            <div className="flex items-center gap-2">
              <span className="text-muted-foreground text-xs">{t("requestedByLabel")}</span>
              <Identity
          ...
          {approval.decisionNote && (
            <p className="text-xs text-muted-foreground">{t("decisionNote")} {approval.decisionNote}</p>
          )}
        </div>
        {error && <p className="text-sm text-destructive">{error}</p>}
        {linkedIssues && linkedIssues.length > 0 && (
          <div className="pt-2 border-t border-border/60">
            <p className="text-xs text-muted-foreground mb-1.5">{t("linkedIssues")}</p>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {isBudgetApproval && approval.status === "pending" && (
            <p className="text-sm text-muted-foreground">
              {t("budgetResolveHint")} <Link to="/costs" className="underline underline-offset-2">{t("budgetResolveLink")}</Link>.
            </p>
          )}
          ...
          {approval.status === "rejected" && approval.type === "hire_agent" && linkedAgentId && (
            <Button
              ...
              onClick={() => {
                if (!window.confirm(t("deleteDisapprovedAgentConfirm"))) return;
                deleteAgentMutation.mutate(linkedAgentId);
              }}
              disabled={deleteAgentMutation.isPending}
            >
              {t("deleteDisapprovedAgent")}
            </Button>
          )}
        </div>
      </div>
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        <h3 className="text-sm font-medium">{t("commentsCount", { count: comments?.length ?? 0 })}</h3>
