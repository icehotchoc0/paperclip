...
const inboxIssueColumnLabelKeys: Record<InboxIssueColumn, string> = {
  status: "columnStatus",
  id: "columnId",
...
const inboxIssueColumnDescriptionKeys: Record<InboxIssueColumn, string> = {
  status: "columnDescStatus",
  id: "columnDescId",
  assignee: "columnDescAssignee",
  project: "columnDescProject",
  workspace: "columnDescWorkspace",
  labels: "columnDescLabels",
  updated: "columnDescUpdated",
};
...
function issueActivityTime(issue: Issue): string {
...
export function FailedRunInboxRow({
  ...
  const displayError = firstNonEmptyLine(run.error) ?? firstNonEmptyLine(run.stderrExcerpt) ?? t("defaultRunError");
        ...
        >
          ...
          <span className="min-w-0 flex-1">
            ...
            <span className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-muted-foreground">
              ...
              <span>{t("updatedTimeAgo", { time: timeAgo(approval.updatedAt) })}</span>
            </span>
          </span>
        </Link>
  ...
  const label =
    joinRequest.requestType === "human"
      ...
      : joinRequest.agentName
        ? t("agentJoinRequestNamed", { name: joinRequest.agentName })
        : t("agentJoinRequest");
  const showUnreadSlot = unreadState !== null;
  const showUnreadDot = unreadState === "visible" || unreadState === "fading";
      ...
      <div className="flex items-start gap-2 sm:items-center">
        ...
        <div className="flex min-w-0 flex-1 items-start gap-2">
          ...
          <span className="min-w-0 flex-1">
            ...
            <span className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-muted-foreground">
              <span>{t("requestedTimeFromIp", { time: timeAgo(joinRequest.createdAt), ip: joinRequest.requestIp })}</span>
              {joinRequest.adapterType && <span>{t("adapterType", { type: joinRequest.adapterType })}</span>}
            </span>
          </span>
        </div>
        <div className="hidden shrink-0 items-center gap-2 sm:flex">
...
export function Inbox() {
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={InboxIcon} message={t("selectCompanyFirst")} />;
  }
        ...
        <Tabs value={tab} onValueChange={(value) => navigate(`/inbox/${value}`)}>
          <PageTabBar
            items={[
              ...
              { value: "unread", label: t("tabUnread") },
        ...
        <div className="flex items-center gap-2">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-[300px] rounded-xl border-border/70 p-1.5 shadow-xl shadow-black/10">
                ...
                >
                  <span className="flex flex-col gap-0.5">
                    <span className="text-sm font-medium text-foreground">
                      {t(inboxIssueColumnLabelKeys[column])}
                    </span>
                    <span className="text-xs leading-relaxed text-muted-foreground">
                      {t(inboxIssueColumnDescriptionKeys[column])}
                    </span>
                  </span>
                </DropdownMenuCheckboxItem>
              ))}
              ...
              >
                {t("resetDefaults")}
                <span className="ml-auto text-xs text-muted-foreground">{t("resetDefaultsHint")}</span>
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
          {canMarkAllRead && (
            <>
              ...
              >
                {markAllReadMutation.isPending ? t("markingAllAsRead") : t("markAllAsRead")}
              </Button>
              <Dialog open={showMarkAllReadConfirm} onOpenChange={setShowMarkAllReadConfirm}>
                <DialogContent className="sm:max-w-md">
                  <DialogHeader>
                    <DialogTitle>{t("markAllAsReadTitle")}</DialogTitle>
                    <DialogDescription>
                      {t("markAllAsReadDescription", { count: unreadIssueIds.length })}
                    </DialogDescription>
                  </DialogHeader>
                  <DialogFooter>
      ...
      {tab === "all" && (
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            ...
            <SelectContent>
              <SelectItem value="everything">{t("categoryAll")}</SelectItem>
              <SelectItem value="issues_i_touched">{t("categoryMyIssues")}</SelectItem>
              <SelectItem value="join_requests">{t("categoryJoinRequests")}</SelectItem>
              <SelectItem value="approvals">{t("categoryApprovals")}</SelectItem>
              <SelectItem value="failed_runs">{t("categoryFailedRuns")}</SelectItem>
              <SelectItem value="alerts">{t("categoryAlerts")}</SelectItem>
            </SelectContent>
          </Select>
          ...
          {showApprovalsCategory && (
            ...
            >
              <SelectTrigger className="h-8 w-[170px] text-xs">
                <SelectValue placeholder={t("approvalFilterAll")} />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">{t("approvalFilterAll")}</SelectItem>
                <SelectItem value="actionable">{t("approvalFilterActionable")}</SelectItem>
                <SelectItem value="resolved">{t("approvalFilterResolved")}</SelectItem>
              </SelectContent>
            </Select>
          )}
        </div>
              ...
              {filteredWorkItems.flatMap((item, index) => {
                ...
                const row = (
                  <IssueRow
                    ...
                    mobileMeta={t("updatedTime", { time: issueActivityTime(issue) }).toLowerCase()}
                    unreadState={
                ...
                <div className="group/alert relative flex items-center gap-3 px-4 py-3 transition-colors hover:bg-accent/50">
                  ...
                  >
                    ...
                    <span className="text-sm">
                      {t("agentErrors", { count: dashboard!.agents.error })}
                    </span>
                  </Link>
                  <button
                    type="button"
              ...
              {showBudgetAlert && (
                <div className="group/alert relative flex items-center gap-3 px-4 py-3 transition-colors hover:bg-accent/50">
                  ...
                  >
                    ...
                    <span className="text-sm">
                      {t("budgetUtilization", { percent: dashboard!.costs.monthUtilizationPercent })}
                    </span>
                  </Link>
                  <button
                    type="button"
