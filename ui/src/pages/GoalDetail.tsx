...
export function GoalDetail() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("breadcrumbGoals"), href: "/goals" },
      { label: goal?.title ?? goalId ?? t("goalFallback") }
    ]);
      ...
      <Tabs defaultValue="children">
        <TabsList>
          <TabsTrigger value="children">
            {t("subGoalsTab", { count: childGoals.length })}
          </TabsTrigger>
          <TabsTrigger value="projects">
            {t("projectsTab", { count: linkedProjects.length })}
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="children" className="mt-4 space-y-3">
          ...
          {childGoals.length === 0 ? (
            <p className="text-sm text-muted-foreground">{t("noSubGoals")}</p>
          ) : (
