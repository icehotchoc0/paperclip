...
export function NewAgent() {
  ...
  const createAgent = useMutation({
    ...
    onError: (error) => {
      setFormError(error instanceof Error ? error.message : t("agents:failedToCreateAgent"));
    },
  });
  ...
  function handleSubmit() {
    ...
    if (configValues.adapterType === "opencode_local") {
      ...
      if (!selectedModel) {
        setFormError(t("agents:openCodeRequiresModel"));
        return;
      }
      if (adapterModelsError) {
        setFormError(
          adapterModelsError instanceof Error
            ...
            : t("agents:failedToLoadOpenCodeModels"),
        );
        return;
      }
      if (adapterModelsLoading || adapterModelsFetching) {
        setFormError(t("agents:openCodeModelsLoading"));
        return;
      }
      ...
      if (!discovered.some((entry) => entry.id === selectedModel)) {
        setFormError(
          discovered.length === 0
            ? t("agents:noOpenCodeModels")
            : t("agents:openCodeModelUnavailable", { model: selectedModel }),
        );
        return;
      }
    }
    ...
    <div className="mx-auto max-w-2xl space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          {t("agents:advancedAgentConfiguration")}
        </p>
      </div>
      ...
      <div className="border border-border">
        ...
        <div className="border-t border-border px-4 py-3">
          {isFirstAgent && (
            <p className="text-xs text-muted-foreground mb-2">{t("agents:thisBeCeo")}</p>
          )}
          {formError && (
