...
export function CompanySettings() {
  ...
  const feedbackSharingMutation = useMutation({
    ...
    onError: (err) => {
      pushToast({
        ...
        body: err instanceof Error ? err.message : t("common:unknown"),
        tone: "error",
      });
    },
  });
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: selectedCompany?.name ?? t("common:selectCompany"), href: "/dashboard" },
    ...
    <div className="max-w-2xl space-y-6">
        ...
        <div className="space-y-3 rounded-md border border-border px-4 py-4">
          ...
          >
            <input
              ...
              type="text"
              value={description}
              placeholder={t("optionalCompanyDescription")}
                ...
                <div className="space-y-2">
                  ...
                  {logoUrl && (
                    <div className="flex items-center gap-2">
                      ...
                      >
                        {clearLogoMutation.isPending ? t("removingLogo") : t("removeLogo")}
                      </Button>
                    </div>
                  )}
                  ...
                  {logoUploadMutation.isPending && (
                    <span className="text-xs text-muted-foreground">{t("uploadingLogo")}</span>
                  )}
                </div>
              </Field>
              <Field
      ...
      <div className="space-y-4" data-testid="company-settings-team-section">
        ...
        <div className="rounded-md border border-border px-4 py-3">
          <ToggleField
            label={t("requireBoardApproval")}
            hint={t("requireBoardApprovalHint")}
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-3 rounded-md border border-border px-4 py-4">
          <ToggleField
            label={t("allowSharingVotedOutputs")}
            hint={t("allowSharingVotedOutputsHint")}
            checked={!!selectedCompany.feedbackDataSharingEnabled}
            onChange={(enabled) => feedbackSharingMutation.mutate(enabled)}
          />
          <p className="text-sm text-muted-foreground">
            {t("votesAlwaysSavedLocally")}
          </p>
          <div className="space-y-1 text-xs text-muted-foreground">
            <div>
              {t("termsVersion")}: {selectedCompany.feedbackDataSharingTermsVersion ?? DEFAULT_FEEDBACK_DATA_SHARING_TERMS_VERSION}
            </div>
            {selectedCompany.feedbackDataSharingConsentAt ? (
              <div>
                ...
                {selectedCompany.feedbackDataSharingConsentByUserId
                  ? ` ${t("enabledBy", { userId: selectedCompany.feedbackDataSharingConsentByUserId })}`
                  : ""}
              </div>
            ) : (
              <div>{t("sharingCurrentlyDisabled")}</div>
            )}
            {FEEDBACK_TERMS_URL ? (
              <a
        ...
        <div className="space-y-3 rounded-md border border-border px-4 py-4">
          <div className="flex items-center gap-1.5">
            <span className="text-xs text-muted-foreground">
              {t("generateInviteSnippetHint")}
            </span>
            <HintIcon text={t("inviteHintTooltip")} />
          </div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              {inviteMutation.isPending
                ? t("generating")
                : t("generateOpenClawInvitePrompt")}
            </Button>
          </div>
          {inviteError && (
      ...
      <div className="space-y-4">
        ...
        <div className="rounded-md border border-border px-4 py-4">
          <p className="text-sm text-muted-foreground">
            <Trans
              i18nKey="importExportMoved"
              ns="settings"
              components={{ orgLink: <a href="/org" className="underline hover:text-foreground" /> }}
            />
          </p>
            ...
            <Button
              ...
              onClick={() => {
                ...
                const confirmed = window.confirm(
                  t("archiveConfirmation", { name: selectedCompany.name })
                );
                if (!confirmed) return;
                const nextCompanyId =
                  companies.find(
