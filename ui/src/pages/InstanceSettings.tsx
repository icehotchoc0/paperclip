...
export function InstanceSettings() {
  ...
  const toggleMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("failedToUpdateHeartbeat"));
    },
  });
  ...
  const disableAllMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("failedToDisableAllHeartbeats"));
    },
  });
  ...
  if (heartbeatsQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("loadingSchedulerHeartbeats")}</div>;
  }
    ...
    return (
      <div className="text-sm text-destructive">
        {heartbeatsQuery.error instanceof Error
          ...
          : t("failedToLoadSchedulerHeartbeats")}
      </div>
    );
  }
    ...
    <div className="max-w-5xl space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("schedulerHeartbeats")}</h1>
        </div>
        <p className="text-sm text-muted-foreground">
          {t("schedulerHeartbeatsDescription")}
        </p>
      </div>
      ...
      <div className="flex items-center gap-4 text-sm text-muted-foreground">
        ...
        <span><span className="font-semibold text-foreground">{grouped.length}</span> {t("company", { count: grouped.length })}</span>
        {anyEnabled && (
          <Button
            variant="destructive"
            size="sm"
            ...
            onClick={() => {
              ...
              if (!window.confirm(t("disableAllConfirmation", { count: enabledCount, noun }))) {
                return;
              }
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="ml-auto flex items-center gap-1.5 shrink-0">
                          ...
                          >
                            {saving ? "..." : agent.heartbeatEnabled ? t("disableTimerHeartbeat") : t("enableTimerHeartbeat")}
                          </Button>
                        </span>
                      </div>
                    );
