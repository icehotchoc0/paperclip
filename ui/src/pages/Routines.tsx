...
const concurrencyPolicyKeys: Record<string, string> = {
  coalesce_if_active: "concurrencyCoalesceIfActive",
  always_enqueue: "concurrencyAlwaysEnqueue",
  skip_if_active: "concurrencySkipIfActive",
};
const catchUpPolicyKeys: Record<string, string> = {
  skip_missed: "catchUpSkipMissed",
  enqueue_missed_with_cap: "catchUpEnqueueMissedWithCap",
};
...
export function Routines() {
  ...
  const createRoutine = useMutation({
    ...
    onSuccess: async (routine) => {
      ...
      pushToast({
        title: t("routineCreatedTitle"),
        body: t("routineCreatedBody"),
        tone: "success",
      });
  ...
  const updateRoutineStatus = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        ...
        body: mutationError instanceof Error ? mutationError.message : t("failedToUpdateBody"),
        tone: "error",
      });
    },
  });
            ...
            <div className="flex flex-col gap-2 sm:items-end">
              ...
              {createRoutine.isError ? (
                <p className="text-sm text-destructive">
                  {createRoutine.error instanceof Error ? createRoutine.error.message : t("failedToCreate")}
                </p>
              ) : null}
            </div>
          </div>
            ...
            <table className="min-w-full text-sm">
              <thead>
                <tr className="text-left text-xs text-muted-foreground border-b border-border">
                  <th className="px-3 py-2 font-medium">{t("tableHeaderName")}</th>
                  <th className="px-3 py-2 font-medium">{t("tableHeaderProject")}</th>
                  <th className="px-3 py-2 font-medium">{t("tableHeaderAgent")}</th>
                  <th className="px-3 py-2 font-medium">{t("tableHeaderLastRun")}</th>
                  <th className="px-3 py-2 font-medium">{t("tableHeaderEnabled")}</th>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-3 py-2.5 text-muted-foreground">
                        <div>{formatLastRunTimestamp(routine.lastRun?.triggeredAt, t("never"))}</div>
                        {routine.lastRun ? (
                      ...
                      <td className="px-3 py-2.5" onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center gap-3">
                          <button
                            ...
                            aria-label={enabled ? t("disable", { title: routine.title }) : t("enableRoutine", { title: routine.title })}
                          ...
                          <span className="text-xs text-muted-foreground">
                            {isArchived ? t("archivedCapital") : enabled ? t("on") : t("off")}
                          </span>
                        </div>
                      </td>
                      <td className="px-3 py-2.5 text-right" onClick={(e) => e.stopPropagation()}>
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            >
                              {runningRoutineId === routine.id ? t("runningEllipsis") : t("runNow")}
                            </DropdownMenuItem>
                            <DropdownMenuSeparator />
                            <DropdownMenuItem
                              onClick={() =>
