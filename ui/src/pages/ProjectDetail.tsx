  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
        ...
        {project.targetDate && (
          <div>
            <span className="text-muted-foreground">{t("targetDateLabel")}</span>
...
function ProjectWorkspacesContent({
  ...
  if (summaries.length === 0) {
    return <p className="text-sm text-muted-foreground">{t("noWorkspaceActivityYet")}</p>;
  }
      ...
      >
          ...
          <div className="ml-auto flex shrink-0 items-center gap-2">
            ...
            {summary.hasRuntimeConfig ? (
              ...
              >
                ...
                {hasRunningServices ? t("stopButton") : t("startButton")}
              </Button>
            ) : null}
            {summary.kind === "execution_workspace" && summary.executionWorkspaceId && summary.executionWorkspaceStatus ? (
              ...
              >
                {summary.executionWorkspaceStatus === "cleanup_failed" ? t("retryClose") : t("common:close")}
              </Button>
            ) : null}
          </div>
        </div>
        ...
        {summary.issues.length > 0 ? (
          <div className="mt-2 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-muted-foreground">
            <span className="font-medium text-muted-foreground/70">{t("issuesLabel")}</span>
            ...
            {hiddenIssueCount > 0 ? (
              <Link to={workspaceHref} className="hover:text-foreground hover:underline">
                {t("moreIssues", { count: hiddenIssueCount })}
              </Link>
            ) : null}
          </div>
        ) : null}
...
export function ProjectDetail() {
  ...
  const archiveProject = useMutation({
    ...
    onSuccess: (updatedProject, archived) => {
      ...
      if (archived) {
        pushToast({ title: t("hasBeenArchived", { name }), tone: "success" });
        navigate("/dashboard");
      } else {
        pushToast({ title: t("hasBeenUnarchived", { name }), tone: "success" });
      }
    },
    onError: (_, archived) => {
      pushToast({
        title: archived ? t("failedToArchiveProject") : t("failedToUnarchiveProject"),
        tone: "error",
      });
    },
  });
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: project?.name ?? routeProjectRef ?? t("breadcrumbProjects") },
    ]);
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs value={activeTab ?? "list"} onValueChange={(value) => handleTabChange(value as ProjectTab)}>
        <PageTabBar
          items={[
            { value: "list", label: t("issuesTab") },
            { value: "overview", label: t("overviewTab") },
            ...(showWorkspacesTab ? [{ value: "workspaces", label: t("workspacesTab") }] : []),
            { value: "configuration", label: t("configurationTab") },
            { value: "budget", label: t("budgetTab") },
