    ...
    <div className="border-b border-border px-4 py-4">
      <div className="space-y-3">
        ...
        <Input
          ...
          placeholder={t("optionalShortname")}
          className="h-9 rounded-none border-0 border-b border-border px-0 shadow-none focus-visible:ring-0"
        />
        <Textarea
          value={description}
        ...
        <div className="flex items-center justify-end gap-2">
          ...
          >
            {isPending ? t("creating") : t("createSkill")}
          </Button>
        </div>
      </div>
    </div>
    ...
    <div className="min-w-0">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          {detail.editable ? (
            ...
            >
              ...
              {editMode ? t("stopEditing") : t("common:edit")}
            </button>
          ) : (
        ...
        <div className="mt-4 space-y-3 border-t border-border pt-4 text-sm">
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
                ...
                {detail.sourcePath ? (
                  <button
                    ...
                    onClick={() => {
                      ...
                      pushToast({ title: t("copiedPathToWorkspace") });
                    }}
                  >
                    {source.label}
                  </button>
            ...
            {detail.sourceType === "github" && (
              <div className="flex flex-wrap items-center gap-2">
                ...
                {updateStatus?.trackingRef && (
                  <span className="text-xs text-muted-foreground">{t("tracking", { ref: updateStatus.trackingRef })}</span>
                )}
                <Button
                  variant="ghost"
                  size="sm"
                ...
                {updateStatus?.supported && updateStatus.hasUpdate && (
                  ...
                  >
                    ...
                    {latestPin ? t("installUpdateWithRef", { ref: latestPin }) : t("installUpdate")}
                  </Button>
                )}
            ...
            <div className="flex items-center gap-2">
              ...
              <span>{detail.editable ? t("editable") : t("readOnly")}</span>
            </div>
          </div>
          <div className="flex flex-wrap items-start gap-x-3 gap-y-1">
            <span className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("usedBy")}</span>
            {usedBy.length === 0 ? (
              <span className="text-muted-foreground">{t("noAgentsAttached")}</span>
            ) : (
            ...
            {editMode && file?.editable && (
              <>
                ...
                <Button size="sm" onClick={onSave} disabled={savePending}>
                  ...
                  {savePending ? t("saving") : t("common:save")}
                </Button>
              </>
            )}
          </div>
      ...
      <div className="min-h-[560px] px-5 py-5">
        {fileLoading ? (
          ...
          <div className="text-sm text-muted-foreground">{t("selectFileToInspect")}</div>
        ) : editMode && file.editable ? (
          file.markdown ? (
            <MarkdownEditor
              value={draft}
...
export function CompanySkills() {
  ...
  const importSkill = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        tone: "success",
        title: t("skillsImported"),
        body: t("skillsImportedBody", { count: result.imported.length }),
      });
      if (result.warnings[0]) {
        pushToast({ tone: "warn", title: t("importWarnings"), body: result.warnings[0] });
      }
      setSource("");
    },
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("skillImportFailed"),
        body: error instanceof Error ? error.message : t("failedToImportSkillSource"),
      });
    },
  });
  ...
  const createSkill = useMutation({
    ...
    onSuccess: async (skill) => {
      ...
      pushToast({
        ...
        body: t("skillCreatedBody", { name: skill.name }),
      });
    },
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("skillCreationFailed"),
        body: error instanceof Error ? error.message : t("failedToCreateSkill"),
      });
    },
  });
  ...
  const scanProjects = useMutation({
    ...
    onMutate: () => {
      setScanStatusMessage(t("scanningProjectWorkspaces"));
    },
    onSuccess: async (result) => {
      setScanStatusMessage(t("refreshingSkillsList"));
      await queryClient.invalidateQueries({ queryKey: queryKeys.companySkills.list(selectedCompanyId!) });
      const summary = formatProjectScanSummary(result);
      setScanStatusMessage(summary);
      pushToast({
        tone: "success",
        title: t("projectSkillScanComplete"),
        body: summary,
      });
      if (result.conflicts[0]) {
        pushToast({
          tone: "warn",
          title: t("skillConflictsFound"),
          body: result.conflicts[0].reason,
        });
      } else if (result.warnings[0]) {
        pushToast({
          tone: "warn",
          title: t("scanWarnings"),
    ...
    onError: (error) => {
      ...
      pushToast({
        tone: "error",
        title: t("projectSkillScanFailed"),
        body: error instanceof Error ? error.message : t("failedToScanProjectWorkspaces"),
      });
    },
  });
  ...
  const saveFile = useMutation({
    ...
    onError: (error) => {
      pushToast({
        tone: "error",
        title: t("saveFailed"),
        body: error instanceof Error ? error.message : t("failedToSaveSkillFile"),
      });
    },
  });
  ...
  const installUpdate = useMutation({
    ...
    onSuccess: async (skill) => {
      ...
      pushToast({
        tone: "success",
        title: t("skillUpdated"),
        body: skill.sourceRef ? t("pinnedTo", { ref: shortRef(skill.sourceRef) }) : skill.name,
      });
    },
    onError: (error) => {
      pushToast({
        ...
        body: error instanceof Error ? error.message : t("failedToInstallSkillUpdate"),
      });
    },
  });
    ...
    <>
      <Dialog open={emptySourceHelpOpen} onOpenChange={setEmptySourceHelpOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{t("addSkillSource")}</DialogTitle>
            <DialogDescription>
              {t("addSkillSourceDescription")}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 text-sm">
            ...
            >
              <span>
                <span className="block font-medium">{t("browseSkillsSh")}</span>
                <span className="mt-1 block text-muted-foreground">
                  {t("browseSkillsShDescription")}
                </span>
              </span>
            ...
            >
              <span>
                <span className="block font-medium">{t("searchGitHub")}</span>
                <span className="mt-1 block text-muted-foreground">
                  {t("searchGitHubDescription")}
                </span>
              </span>
              <ExternalLink className="mt-0.5 h-4 w-4 shrink-0 text-muted-foreground" />
            </a>
