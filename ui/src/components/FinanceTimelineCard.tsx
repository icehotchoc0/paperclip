...
export function FinanceTimelineCard({
  ...
  const resolvedEmptyMessage = emptyMessage ?? t("noFinancialEventsInPeriod");
  return (
    <Card>
      <CardHeader className="px-4 pt-4 pb-1">
        <CardTitle className="text-base">{t("recentFinancialEvents")}</CardTitle>
        <CardDescription>{t("recentFinancialEventsDescription")}</CardDescription>
      </CardHeader>
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0 space-y-2">
                  ...
                  {(row.description || row.externalInvoiceId || row.region || row.pricingTier) && (
                    <div className="space-y-1 text-xs text-muted-foreground">
                      ...
                      {row.externalInvoiceId ? <div>{t("invoice", { id: row.externalInvoiceId })}</div> : null}
                      {row.region ? <div>{t("region", { name: row.region })}</div> : null}
                      {row.pricingTier ? <div>{t("tier", { name: row.pricingTier })}</div> : null}
                    </div>
                  )}
                </div>
                <div className="text-right tabular-nums">
                  ...
                  {row.estimated ? <div className="text-[11px] uppercase tracking-[0.12em] text-amber-600">{t("estimatedLabel")}</div> : null}
                </div>
              </div>
            </div>
          ))
