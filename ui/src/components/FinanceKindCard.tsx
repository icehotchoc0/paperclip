  ...
  return (
    <Card>
      <CardHeader className="px-4 pt-4 pb-1">
        <CardTitle className="text-base">{t("financialEventMix")}</CardTitle>
        <CardDescription>{t("financialEventMixDescription")}</CardDescription>
      </CardHeader>
      <CardContent className="space-y-2 px-4 pb-4 pt-3">
        {rows.length === 0 ? (
          <p className="text-sm text-muted-foreground">{t("noFinanceEventsInPeriod")}</p>
        ) : (
          rows.map((row) => (
            <div
              key={row.eventKind}
            ...
            >
              <div className="min-w-0">
                ...
                <div className="text-xs text-muted-foreground">
                  {t("events", { count: row.eventCount })} · {t("billers", { count: row.billerCount })}
                </div>
              </div>
              <div className="text-right tabular-nums">
                <div className="text-sm font-medium">{formatCents(row.netCents)}</div>
                <div className="text-xs text-muted-foreground">
                  {t("debitsAmount", { amount: formatCents(row.debitCents) })}
                </div>
              </div>
            </div>
          ))
