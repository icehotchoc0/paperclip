  ...
  return (
    <Dialog open={open} onOpenChange={(next) => !isPending && onOpenChange(next)}>
      <DialogContent className="max-w-xl">
        <DialogHeader>
          <DialogTitle>{t("runRoutineDialog")}</DialogTitle>
          <DialogDescription>
            {t("runRoutineDialogDescription")}
          </DialogDescription>
        </DialogHeader>
            ...
            <div key={variable.name} className="space-y-1.5">
              ...
              {variable.type === "textarea" ? (
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="__unset__">{t("noValue")}</SelectItem>
                ...
                >
                  <SelectTrigger>
                    <SelectValue placeholder={t("chooseAValue")} />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="__unset__">{t("noValue")}</SelectItem>
        ...
        <DialogFooter showCloseButton={false}>
          {missingRequired.length > 0 ? (
            <p className="mr-auto text-xs text-amber-600">
              {t("missing", { fields: missingRequired.join(", ") })}
            </p>
          ) : workspaceSelectionEnabled && !workspaceConfigValid ? (
            <p className="mr-auto text-xs text-amber-600">
              {t("chooseWorkspaceBefore")}
            </p>
          ) : (
          ...
          >
            {isPending ? t("runningEllipsis") : t("runRoutine")}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
