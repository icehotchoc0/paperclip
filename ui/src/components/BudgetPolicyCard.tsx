...
function useWindowLabel() {
  ...
  return (windowKind: BudgetPolicySummary["windowKind"]) =>
    windowKind === "lifetime" ? t("lifetimeBudget") : t("monthlyUtcBudget");
}
...
export function BudgetPolicyCard({
    ...
    <div className="grid gap-6 sm:grid-cols-2">
      <div>
        ...
        <div className="mt-1 text-xs text-muted-foreground">
          {summary.amount > 0 ? t("percentOfLimit", { percent: summary.utilizationPercent }) : t("noCapConfigured")}
        </div>
      </div>
      <div>
        ...
        <div className="mt-1 text-xs text-muted-foreground">
          {t("softAlertAt", { percent: summary.warnPercent })}{summary.paused && summary.pauseReason ? ` · ${t("pauseReason", { reason: summary.pauseReason })}` : ""}
        </div>
      </div>
    </div>
  ) : (
    ...
    <div className="flex items-start gap-2 rounded-xl border border-red-500/30 bg-red-500/10 px-3 py-2 text-sm text-red-100">
      ...
      <div>
        {summary.scopeType === "project"
          ? t("projectPausedUntilResolved")
          : t("scopePausedUntilResolved")}
      </div>
    </div>
  ) : null;
  ...
  const saveSection = onSave ? (
    <div className={cn("flex flex-col gap-3 sm:flex-row sm:items-end", isPlain ? "" : "rounded-xl border border-border/70 bg-background/50 p-3")}>
      ...
      >
        {isSaving ? t("saving") : summary.amount > 0 ? t("updateBudget") : t("setBudget")}
      </Button>
    </div>
  ) : null;
      ...
      <div className="space-y-6">
        <div className="flex items-start justify-between gap-6">
          ...
          >
            ...
            {summary.paused ? t("paused") : summary.status === "warning" ? t("warning") : summary.status === "hard_stop" ? t("hardStopStatus") : t("healthy")}
          </div>
        </div>
        ...
        {saveSection}
        {parsedDraft === null ? (
          <p className="text-xs text-destructive">{t("validDollarAmount")}</p>
        ) : null}
      </div>
    );
  }
