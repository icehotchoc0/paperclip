    ...
    <>
      ...
      {collectingDownvoteReason ? (
        <div className="mt-2 rounded-md border border-border/60 bg-accent/20 p-3">
          <div className="mb-2 text-sm font-medium">{t("whatCouldHaveBeenBetter")}</div>
          <Textarea
            value={downvoteReason}
            onChange={(event) => setDownvoteReason(event.target.value)}
            placeholder={t("addAShortNote")}
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("saveFeedbackPreference")}</DialogTitle>
            <DialogDescription>
              {t("feedbackSharingDescription")}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 text-sm text-muted-foreground">
            <p>
              {t("voteAlwaysSavedLocally")}
            </p>
            <p>
              Choose <span className="font-medium text-foreground">{t("alwaysAllow")}</span> to share
              this vote and future voted AI outputs. Choose{" "}
              <span className="font-medium text-foreground">{t("dontAllow")}</span> to keep this vote
          ...
          <DialogFooter>
            ...
            >
              {isSaving ? t("saving") : t("alwaysAllow")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
