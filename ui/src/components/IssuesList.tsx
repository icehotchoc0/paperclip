...
const quickFilterPresets = [
  { i18nKey: "filterAll", statuses: [] as string[] },
  { i18nKey: "filterActive", statuses: ["todo", "in_progress", "in_review", "blocked"] },
  { i18nKey: "filterBacklog", statuses: ["backlog"] },
  { i18nKey: "filterDone", statuses: ["done", "cancelled"] },
];
      ...
      label:
        key === "__unassigned"
          ...
          : key.startsWith("__user:")
            ? (formatAssigneeUserLabel(key.slice("__user:".length), currentUserId) ?? t("user"))
  ...
  return (
    <div className="space-y-4">
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="ghost" size="sm" className={`text-xs ${activeFilterCount > 0 ? "text-blue-600 dark:text-blue-400" : ""}`}>
                ...
                <span className="hidden sm:inline">{activeFilterCount > 0 ? t("filtersCount", { count: activeFilterCount }) : t("common:filter")}</span>
                {activeFilterCount > 0 && (
            ...
            <PopoverContent align="end" className="w-[min(480px,calc(100vw-2rem))] p-0">
              <div className="p-3 space-y-3">
                ...
                <div className="space-y-1.5">
                  <span className="text-xs text-muted-foreground">{t("quickFilters")}</span>
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="space-y-0.5 max-h-32 overflow-y-auto">
                        <label className="flex items-center gap-2 px-2 py-1 rounded-sm hover:bg-accent/50 cursor-pointer">
                          ...
                          />
                          <span className="text-sm">{t("noAssignee")}</span>
                        </label>
                        {currentUserId && (
                          <label className="flex items-center gap-2 px-2 py-1 rounded-sm hover:bg-accent/50 cursor-pointer">
                            <Checkbox
              ...
              <PopoverContent align="end" className="w-48 p-0">
                <div className="p-2 space-y-0.5">
                  {(["status", "priority", "title", "created", "updated"] as const).map((field) => {
                    const label = { status: t("sortStatus"), priority: t("sortPriority"), title: t("sortTitle"), created: t("sortCreated"), updated: t("sortUpdated") }[field];
                    return (
                    <button
                      key={field}
              ...
              <PopoverContent align="end" className="w-44 p-0">
                <div className="p-2 space-y-0.5">
                  {(["status", "priority", "assignee", "none"] as const).map((value) => {
                    const label = { status: t("groupStatus"), priority: t("groupPriority"), assignee: t("groupAssignee"), none: t("groupNone") }[value];
                    return (
                    <button
                      key={value}
                      className={`flex items-center justify-between w-full px-2 py-1.5 text-sm rounded-sm ${
      ...
      {!isLoading && filtered.length === 0 && viewState.viewMode === "list" && (
        <EmptyState
          icon={CircleDot}
          message={t("noIssuesMatchFilters")}
          action={t("createIssue")}
