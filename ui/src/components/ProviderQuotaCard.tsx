        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <CardDescription className="text-xs mt-0.5">
              <span className="font-mono">{formatTokens(totalInputTokens)}</span> {t("tokensIn")}
              {" · "}
              <span className="font-mono">{formatTokens(totalOutputTokens)}</span> {t("tokensOut")}
        ...
        {hasBudget && (
          <div className="space-y-3">
            ...
            <QuotaBar
              ...
              rightLabel={t("perWeek", { amount: formatCents(Math.round(weeklyBudgetShare)) })}
          ...
          <>
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                ...
                <span className="font-mono text-foreground">{formatTokens(totalSubInputTokens)}</span> {t("tokensIn")}
                {" · "}
                <span className="font-mono text-foreground">{formatTokens(totalSubOutputTokens)}</span> {t("tokensOut")}
              </p>
              {subSharePct > 0 && (
                <>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {t("percentViaSubscription", { percent: Math.round(subSharePct) })}
                  </p>
                </>
              )}
            </div>
                  ...
                  <div key={`${row.provider}:${row.model}`} className="space-y-1.5">
                    ...
                    <div className="relative h-2 w-full border border-border overflow-hidden">
                      <div
                        ...
                        title={t("percentOfProviderTokens", { percent: Math.round(tokenPct) })}
                      />
                      ...
                      <div
                        ...
                        style={{ width: `${costPct}%` }}
                        title={t("percentOfProviderCost", { percent: Math.round(costPct) })}
                      />
                    </div>
                  </div>
                );
                        ...
                        <div key={qw.label} className="space-y-1">
                          <div className="flex items-center justify-between gap-2 text-xs">
                            ...
                            {qw.valueLabel != null ? (
                              ...
                              <span className="font-medium tabular-nums">{t("percentUsed", { percent: qw.usedPercent })}</span>
                            ) : null}
                          </div>
                          ...
                          {qw.detail ? (
                            ...
                            <p className="text-xs text-muted-foreground">
                              {t("resets", { date: new Date(qw.resetsAt).toLocaleDateString(undefined, { month: "short", day: "numeric" }) })}
                            </p>
                          ) : null}
                        </div>
                      );
