...
export function NewProjectDialog() {
  ...
  async function handleSubmit() {
    ...
    if (localPath && !isAbsolutePath(localPath)) {
      setWorkspaceError(t("localFolderMustBeAbsolute"));
      return;
    }
    if (repoUrl && !looksLikeRepoUrl(repoUrl)) {
      setWorkspaceError(t("repoMustBeValidUrl"));
      return;
    }
      ...
      >
        ...
        <div className="px-4 pb-2">
          <MarkdownEditor
            ...
            placeholder={t("addDescriptionEllipsis")}
            bordered={false}
        ...
        <div className="px-4 pt-3 pb-3 space-y-3 border-t border-border">
          <div>
            <div className="mb-1 flex items-center gap-1.5">
              <label className="block text-xs text-muted-foreground">{t("repoUrlLabel")}</label>
              <span className="text-xs text-muted-foreground/50">{t("repoUrlOptional")}</span>
          ...
          <div>
            <div className="mb-1 flex items-center gap-1.5">
              <label className="block text-xs text-muted-foreground">{t("localFolderLabel")}</label>
              <span className="text-xs text-muted-foreground/50">{t("localFolderOptional")}</span>
              <Tooltip delayDuration={300}>
                <TooltipTrigger asChild>
                  <HelpCircle className="h-3 w-3 text-muted-foreground/50 cursor-help" />
                </TooltipTrigger>
                <TooltipContent side="top" className="max-w-[240px] text-xs">
                  {t("localFolderTooltip")}
                </TooltipContent>
              </Tooltip>
            </div>
          ...
          <Popover open={goalOpen} onOpenChange={setGoalOpen}>
            <PopoverTrigger asChild>
              ...
              >
                ...
                {selectedGoals.length > 0 ? t("plusGoal") : t("goalLabel")}
              </button>
            </PopoverTrigger>
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-t border-border">
          {createProject.isError ? (
            <p className="text-xs text-destructive">{t("failedToCreateProject")}</p>
          ) : (
            <span />
          )}
          <Button
