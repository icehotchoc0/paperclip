...
function ArchiveDangerZone({
  ...
  const action = isArchive ? t("archiveProject") : t("unarchiveProject");
      ...
      {archivePending ? (
        ...
        <div className="flex items-center gap-2">
          <span className="text-sm text-destructive font-medium">
            {t("archiveConfirm", { action, name: project.name })}
          </span>
          <Button
            size="sm"
            variant="destructive"
...
export function ProjectProperties({ project, onUpdate, onFieldUpdate, getFieldSaveState, onArchive, archivePending }: ProjectPropertiesProps) {
  ...
  const submitLocalWorkspace = () => {
    ...
    if (!isAbsolutePath(cwd)) {
      setWorkspaceError(t("localFolderAbsoluteError"));
      return;
    }
    setWorkspaceError(null);
    persistCodebase({ cwd });
  ...
  const submitRepoWorkspace = () => {
    ...
    if (!looksLikeRepoUrl(repoUrl)) {
      setWorkspaceError(t("repoValidUrlError"));
      return;
    }
    setWorkspaceError(null);
  ...
  const clearLocalWorkspace = () => {
    const confirmed = window.confirm(
      codebase.repoUrl
        ? t("clearLocalFolderConfirm")
        : t("deleteLocalFolderConfirm"),
    );
    if (!confirmed) return;
    persistCodebase({ cwd: null });
  };
  ...
  return (
    <div>
      <div className="space-y-1 pb-4">
        <PropertyRow label={<FieldLabel label={t("nameFieldLabel")} state={fieldState("name")} />}>
        ...
        <PropertyRow
          label={<FieldLabel label={t("descriptionFieldLabel")} state={fieldState("description")} />}
          alignStart
        ...
        <PropertyRow label={<FieldLabel label={t("statusFieldLabel")} state={fieldState("status")} />}>
          {onUpdate || onFieldUpdate ? (
            <ProjectStatusPicker
        ...
        {project.leadAgentId && (
          <PropertyRow label={t("leadLabel")}>
            <span className="text-sm font-mono">{project.leadAgentId.slice(0, 8)}</span>
          </PropertyRow>
        )}
        <PropertyRow
          label={<FieldLabel label={t("goalsFieldLabel")} state={fieldState("goals")} />}
          alignStart
        ...
        <PropertyRow label={<FieldLabel label={t("createdFieldLabel")} state="idle" />}>
          <span className="text-sm">{formatDate(project.createdAt)}</span>
        </PropertyRow>
        <PropertyRow label={<FieldLabel label={t("updatedFieldLabel")} state="idle" />}>
          <span className="text-sm">{formatDate(project.updatedAt)}</span>
        </PropertyRow>
        {project.targetDate && (
          <PropertyRow label={<FieldLabel label={t("targetDateFieldLabel")} state="idle" />}>
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-2 rounded-md border border-border/70 p-3">
            <div className="space-y-1">
              <div className="text-[11px] uppercase tracking-wide text-muted-foreground">{t("repoSectionLabel")}</div>
              {codebase.repoUrl ? (
                ...
                <div className="flex items-center justify-between gap-2">
                  <div className="text-xs text-muted-foreground">{t("notSetValue")}</div>
                  <Button
                    variant="outline"
                    size="xs"
                    className="h-6 px-2"
            ...
            <div className="space-y-1">
              <div className="text-[11px] uppercase tracking-wide text-muted-foreground">{t("localFolderSectionLabel")}</div>
              <div className="flex items-center justify-between gap-2">
                <div className="min-w-0 space-y-1">
                  ...
                  {codebase.origin === "managed_checkout" && (
                    <div className="text-[11px] text-muted-foreground">{t("paperclipManagedFolder")}</div>
                  )}
                </div>
                <div className="flex items-center gap-1">
                  <Button
                  ...
                  >
                    {codebase.localFolder ? t("changeLocalFolder") : t("setLocalFolder")}
                  </Button>
          ...
          {createWorkspace.isError && (
            <p className="text-xs text-destructive">{t("failedToSaveWorkspaceShort")}</p>
          )}
          {removeWorkspace.isError && (
            <p className="text-xs text-destructive">{t("failedToDeleteWorkspace")}</p>
          )}
          {updateWorkspace.isError && (
            <p className="text-xs text-destructive">{t("failedToUpdateWorkspaceShort")}</p>
          )}
        </div>
        ...
        {isolatedWorkspacesEnabled ? (
          <>
            ...
            <div className="py-1.5 space-y-2">
              <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                <span>{t("executionWorkspacesLabel")}</span>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button
                      type="button"
                ...
                <div className="flex items-center justify-between gap-3">
                  <div className="space-y-0.5">
                    <div className="flex items-center gap-2 text-sm font-medium">
                      <span>{t("enableIsolatedCheckouts")}</span>
                      <SaveIndicator state={fieldState("execution_workspace_enabled")} />
                    </div>
                    <div className="text-xs text-muted-foreground">
                      {t("enableIsolatedCheckoutsDescription")}
                    </div>
                  </div>
                  {onUpdate || onFieldUpdate ? (
                    <button
                    ...
                    <span className="text-xs text-muted-foreground">
                      {executionWorkspacesEnabled ? t("enabledLabel") : t("disabledLabel")}
                    </span>
                  )}
                </div>
                ...
                {executionWorkspacesEnabled ? (
                  <div className="space-y-3">
                    <div className="flex items-center justify-between gap-3">
                      <div className="space-y-0.5">
                        <div className="flex items-center gap-2 text-sm">
                          <span>{t("newIssuesDefaultIsolated")}</span>
                          <SaveIndicator state={fieldState("execution_workspace_default_mode")} />
                        </div>
                        <div className="text-[11px] text-muted-foreground">
                          {t("newIssuesDefaultIsolatedDescription")}
                        </div>
                      </div>
                      <button
                        data-slot="toggle"
                    ...
                    <div className="border-t border-border/60 pt-2">
                      ...
                      >
                        {executionWorkspaceAdvancedOpen
                          ? t("hideAdvancedCheckoutSettings")
                          : t("showAdvancedCheckoutSettings")}
                      </button>
                    </div>
                    ...
                    {executionWorkspaceAdvancedOpen ? (
                      <div className="space-y-3">
                        <div className="text-xs text-muted-foreground">
                          {t("hostManagedImplementation")}<span className="text-foreground">{t("gitWorktree")}</span>
                        </div>
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("baseRefFieldLabel")}</span>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("branchTemplateLabel")}</span>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("worktreeParentDirLabel")}</span>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("provisionCommandFieldLabel")}</span>
                        ...
                        <div>
                          <div className="mb-1 flex items-center gap-1.5">
                            <label className="flex items-center gap-2 text-xs text-muted-foreground">
                              <span>{t("teardownCommandFieldLabel")}</span>
