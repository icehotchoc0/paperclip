    ...
    <div>
      <div className="flex items-end gap-[3px] h-20">
        {days.map(day => {
          ...
          return (
            <div key={day} className="flex-1 h-full flex flex-col justify-end" title={t("charts.runTitle", { day, count: total })}>
              {total > 0 ? (
    ...
    <div>
      <div className="flex items-end gap-[3px] h-20">
        {days.map(day => {
          ...
          const heightPct = (total / maxValue) * 100;
          return (
            <div key={day} className="flex-1 h-full flex flex-col justify-end" title={t("charts.issueTitle", { day, count: total })}>
              {total > 0 ? (
                <div className="flex flex-col-reverse gap-px overflow-hidden" style={{ height: `${heightPct}%`, minHeight: 2 }}>
      ...
      <DateLabels days={days} />
      <ChartLegend items={priorityOrder.map(p => ({ color: priorityColors[p], label: t(`common:${p}`, { defaultValue: p.charAt(0).toUpperCase() + p.slice(1) }) }))} />
    </div>
  );
}
...
export function IssueStatusChart({ issues }: { issues: { status: string; createdAt: Date }[] }) {
  ...
  return (
    <div>
      ...
      <DateLabels days={days} />
      <ChartLegend items={statusOrder.map(s => ({ color: statusColors[s] ?? "#6b7280", label: STATUS_LABEL_KEYS[s] ? t(STATUS_LABEL_KEYS[s]) : s }))} />
    </div>
  );
}
    ...
    <div>
      <div className="flex items-end gap-[3px] h-20">
        {days.map(day => {
          ...
          return (
            <div key={day} className="flex-1 h-full flex flex-col justify-end" title={t("charts.successRateTitle", { day, rate: entry.total > 0 ? Math.round(rate * 100) : 0, succeeded: entry.succeeded, total: entry.total })}>
              {entry.total > 0 ? (
