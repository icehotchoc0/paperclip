...
export function ReportsToPicker({
  ...
  const effectiveDisabledEmptyLabel = disabledEmptyLabel ?? t("reportsToNA");
  const effectiveChooseLabel = chooseLabel ?? t("reportsToChoose");
        ...
        >
          {unknownManager ? (
            <>
              ...
              <span className="min-w-0 truncate text-muted-foreground">{t("unknownManager")}</span>
            </>
          ) : current ? (
            <>
              ...
              >
                {terminatedManager ? t("reportsToAgentTerminated", { name: current.name }) : t("reportsToAgent", { name: current.name })}
              </span>
            </>
          ) : (
            <>
